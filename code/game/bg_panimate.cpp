...
//AUSTIN big changes here
saberMoveName_t PM_SaberLungeAttackMove( qboolean fallbackToNormalLunge )
{
    G_DrainPowerForSpecialMove( pm->gent, FP_SABER_OFFENSE, SABER_ALT_ATTACK_POWER_FB );

    // Check if we have an overridden (or cancelled) lunge move
...
default: // normal lunge	AUSTIN lunge move here
    if (fallbackToNormalLunge)
    {
        vec3_t fwdAngles, jumpFwd;

        VectorCopy(pm->ps->viewangles, fwdAngles);
        fwdAngles[PITCH] = fwdAngles[ROLL] = 0;  // Remove pitch and roll to focus on horizontal direction
        AngleVectors(fwdAngles, jumpFwd, NULL, NULL);

        // Only remove jumping animation if the player is in the air
        if (!(pm->ps->groundEntityNum != ENTITYNUM_NONE))  // Check if the player is in the air
        {
            // Clear jump-related flags
            pm->ps->pm_flags &= ~(PMF_JUMPING | PMF_SLOW_MO_FALL); 

            // Capture the current horizontal velocity (ignore vertical component)
            vec3_t currentVelocity = { pm->ps->velocity[0], pm->ps->velocity[1], 0 };  // x and y components only

            // Get the current forward speed of the player (magnitude)
            float forwardSpeed = VectorLength(currentVelocity);
            if (forwardSpeed > 0)
            {
                // Clear out current velocity to avoid carryover
                pm->ps->velocity[0] = 0;
                pm->ps->velocity[1] = 0;

                // Now apply the recorded speed in the new direction
                vec3_t velocityDirection;
                VectorNormalize(jumpFwd);  // Normalize the direction the player is facing
                VectorScale(jumpFwd, forwardSpeed, velocityDirection);  // Apply the speed to the new direction

                // Apply the new velocity to the player
                VectorCopy(velocityDirection, pm->ps->velocity);  // Set the velocity in the direction of the lunge
            }

            // Apply vertical boost for the lunge
            pm->ps->velocity[2] = 50;  // Adjust vertical component if needed
            PM_AddEvent(EV_JUMP);  // Trigger the jump event to reflect the lunge
        }
