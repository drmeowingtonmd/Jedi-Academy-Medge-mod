...
void ForceGrip( gentity_t *self ) //AUSTIN forcegrip/force grip code here
{
    trace_t tr;
    vec3_t end, forward;
    ...
    // After this section, let's add the boost toward the enemy if Force Grip level 2 is active

if ( traceEnt && traceEnt->client )
{
    // Calculate the direction vector from the player to the enemy
    vec3_t direction;
    VectorSubtract(traceEnt->currentOrigin, self->currentOrigin, direction);
    VectorNormalize(direction);  // Normalize the direction vector

    // Determine boost strength based on Force Grip level
    float boostStrength = 0.0f;

    if ( self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_1 )
    {
        boostStrength = 300.0f; // Boost for Force Grip level 1
    }
    else if ( self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_2 )
    {
        boostStrength = 500.0f; // Boost for Force Grip level 2
    }
    else if ( self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_3 )
    {
        boostStrength = 700.0f; // Boost for Force Grip level 3
    }

    // Apply the calculated velocity boost towards the enemy
    if ( boostStrength > 0.0f )
    {
        VectorMA(self->client->ps.velocity, boostStrength, direction, self->client->ps.velocity);
    }
}
    ...
    if ( traceEnt->client ) 
    {
        ...
        {
            if ( traceEnt->s.weapon && traceEnt->client->NPC_class != CLASS_ROCKETTROOPER )
            {
                if ( traceEnt->s.weapon == WP_MELEE )
                {
                }
                else
                {
