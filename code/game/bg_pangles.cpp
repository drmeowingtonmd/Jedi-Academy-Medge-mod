...
qboolean PM_AdjustAngleForWallRun(gentity_t* ent, usercmd_t* ucmd, qboolean doMove)	//AUSTIN wall grab right here!!!
{
		...
		{//still a vertical wall there
			...
			if ((ent->s.number && !G_ControlledByPlayer(ent)) || !player_locked)
			{
				if (doMove)
				{
					...
					vec3_t currentVel;
					VectorCopy(ent->client->ps.velocity, currentVel);

					//pull me toward the wall (only perpendicular component)
					float towardWallForce = -128;
					VectorMA(currentVel, towardWallForce, trace.plane.normal, ent->client->ps.velocity);

					//preserve Z velocity exactly
					ent->client->ps.velocity[2] = currentVel[2];
				}
			}
			ucmd->forwardmove = 0;
			return qtrue;
